...
import { rankings, rankingTabs } from "@/lib/data";
    ...
    >
      <div className="sec-pad mx-auto max-w-[1240px]">
        <div className="mb-10 flex flex-wrap items-baseline justify-between gap-5">
          <SectionHeading zh="天下英雄榜" en="WORLD RANKINGS" />

          <div className="flex flex-wrap gap-2.5" role="tablist" aria-label="英雄榜分类">
            {rankingTabs.map((label, i) => (
              <button
                key={label}
                role="tab"
                aria-selected={i === tab}
        ...
        <div className="gold-grid grid-cols-1 md:grid-cols-3">
          {fighters.map((f) => (
            ...
            >
              <div className="w-11 flex-none font-latin text-[34px] font-bold text-gold/55">
                {f.rank}
              </div>
              ...
              <div className="min-w-0">
                <div className="font-serif-sc text-xl font-black text-white">
                  {f.name}{" "}
                  <span className="text-[13px] font-normal text-flame">「{f.alias}」</span>
                </div>
